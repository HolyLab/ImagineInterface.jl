...
findpositioners(coms::AbstractArray{ImagineCommand,1}) = find(x->length(name(x))>=10 && name(x)[1:10]=="positioner", coms)
getpositioners(coms::AbstractArray{ImagineCommand,1}) = view(coms, findpositioners(coms))

findcameras(coms::AbstractArray{ImagineCommand,1}) = find(x->length(name(x))>=6 && name(x)[1:6]=="camera", coms)
getcameras(coms::AbstractArray{ImagineCommand,1}) = view(coms, findcameras(coms))

findlasers(coms::AbstractArray{ImagineCommand,1}) = find(x->length(name(x))>=5 && name(x)[1:5]=="laser", coms)
getlasers(coms::AbstractArray{ImagineCommand,1}) = view(coms, findlasers(coms))

findstimuli(coms::AbstractArray{ImagineCommand,1}) = find(x->length(name(x))>=8 && name(x)[1:8]=="stimulus", coms)
