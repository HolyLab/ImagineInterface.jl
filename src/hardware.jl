function default_samplemapper(rig_name::String, chan_name::String; samprate = 10000)
...
function piezo_samplemapper{TL<:Unitful.Length,TV<:Voltage}(p::AbstractInterval{TL}, v::AbstractInterval{TV}; rawtype=UInt16, samprate=10000)
    return SampleMapper(typemin(rawtype), typemax(rawtype), minimum(v), maximum(v), minimum(p), maximum(p), 1/samprate * Unitful.s)
end
...
function ttl_samplemapper(; samprate=10000)
    return SampleMapper(UInt8(false), UInt8(true), 0.0*Unitful.V, 3.3*Unitful.V, false, true, 1/samprate * Unitful.s)
end
